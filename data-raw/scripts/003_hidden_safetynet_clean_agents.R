...
future_lapply(
  ...
  function(crop_yr){
    tryCatch({ 
      ...
      agentdata <- agentdata[
        unique(as.data.table(readRDS("data/cleaned_rma_sco_and_eco_adm.rds"))[
          commodity_year %in% year, FCIP_INSURANCE_POOL, with = FALSE]),
        on = FCIP_INSURANCE_POOL, nomatch = 0];gc()
      
      rma_sob <- readRDS("data/cleaned_rma_sobtpu.rds")[commodity_year %in% year]
      rma_sob[
        ,
      ...
      saveRDS(agentdata,file =paste0("data/agentdata/agentdata",year,".rds"))
      rm(agentdata);gc();gc()
      return(year)
